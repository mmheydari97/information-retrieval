import re
import itertools
from search.code.dictionary import news_retrieval, Normalizing, CaseFolding, Tokenization, remove_frequents, Stemming
...
def find(input, phrase):  ## phrase must be 'cat' or 'source'
    ...
    if input.find(phrase + ':') != -1:
        ...
        while (end != len(input) and input[
            end] != ' '):  # note that end!=length must be checked first(To prevent OutOfRange error)
            end += 1
...
def parse_query2(input):
    ...
    words = Stemming(
        remove_frequents(Tokenization(CaseFolding(Normalizing(news_retrieval(word_preprocessor.strip()))))))
    # print(words)
